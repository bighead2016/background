-module(set_activity_tl_channel_recharge).
...
handle(ArgList) ->
	...
	Title1 = 
	case lists:keyfind(<<"title">>, 1, ArgList) of
		false ->
			?ERR(?MODULE, "parma title missing"),
			throw({error, {struct, [{code, <<"1002">>},{message, unicode:characters_to_binary("参数错误")}]}});
		{_, Title0} ->
			util_background:bitstring_to_term(Title0)
	end,
	...
	case lists:keyfind(<<"content">>, 1, ArgList) of
		false ->
			...
			throw({error, {struct, [{code, <<"1002">>},{message, unicode:characters_to_binary("参数错误")}]}});
		{_, Content0} ->
			util_background:bitstring_to_term(Content0)
	end,
