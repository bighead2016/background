-module(set_activity_tl_tot_consume_2).
...
%% http://192.168.1.235:10010/background/set_activity_tl_tot_consume?stage=1&request_diamond=100&item_list=[{1,1}]&flag=
%% 关闭活动，由其他系统设定
handle(ArgList) ->
	Opraction = 
	...
	case cache:lookup(activity_tl_tot_consume_2_cfg, Stage) of
		[] ->
			case Opraction of
				2 ->
					ok;
				_ ->
					cache:insert(#activity_tl_tot_consume_2_cfg{ 
											stage = Stage,
											gift_id = GiftID,
		...
		[TotRecharge] ->
			case Opraction of
				...
				_ ->
					cache:update(TotRecharge#activity_tl_tot_consume_2_cfg{
											stage = Stage,
											gift_id = GiftID,
											unlock_diamond = RequestMoney,
											item_list = ItemList
