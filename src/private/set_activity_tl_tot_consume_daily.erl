-module(set_activity_tl_tot_consume_daily).
...
handle(ArgList) ->
	...
	case cache:lookup(activity_tl_tot_consume_daily_cfg, Stage) of
		[] ->
			case Opraction of
				2 ->
					ok;
				_ ->
					cache:insert(#activity_tl_tot_consume_daily_cfg{ 
											stage = Stage,
											gift_id = GiftID,
		...
		[TotRecharge] ->
			case Opraction of
				...
				_ ->
					cache:update(TotRecharge#activity_tl_tot_consume_daily_cfg{
											stage = Stage,
											gift_id = GiftID,
											unlock_diamond = RequestMoney,
											item_list = ItemList
