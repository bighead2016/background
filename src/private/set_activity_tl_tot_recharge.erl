-module(set_activity_tl_tot_recharge).
...
handle(ArgList) ->
	...
	case cache:lookup(activity_tl_tot_recharge_cfg, Stage) of
		[] ->
			case Opraction of
				2 ->
					ok;
				_ ->
					cache:insert(#activity_tl_tot_recharge_cfg{ 
											stage = Stage,
											gift_id = GiftID,
		...
		[TotRecharge] ->
			case Opraction of
				...
				_ ->
					cache:update(TotRecharge#activity_tl_tot_recharge_cfg{
											stage = Stage,
											gift_id = GiftID,
											unlock_money = RequestMoney,
											item_list = ItemList
